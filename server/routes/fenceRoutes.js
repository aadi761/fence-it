...
router.post('/:id/test', async (req, res) => {
  ...
  const subDoc = await Subscription.findOne({ deviceId });
  if (!subDoc || !subDoc.subscription) return res.status(404).json({ error: 'subscription not found for this device' });

  const sent = await sendPush(subDoc.subscription, {
    source: 'push',
  ...
  if (!sent) return res.status(502).json({ error: 'push send failed' });
